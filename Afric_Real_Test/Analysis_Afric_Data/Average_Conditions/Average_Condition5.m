%% Condition 5
function [S1,S2] = Average_Condition5(Row_info,Col_info,hh,vv,xx)

hh.ref_off = hh.ref((Row_info.row - Row_info.window_row):(Row_info.row + Row_info.window_row)...
    ,(Col_info.col - Col_info.window_col):(Col_info.col + Col_info.window_col));

[row_total,col_total] = size(hh.ref_off); 

window_area = row_total*col_total;

S1.h = reshape(hh.ref_off,1,window_area);
