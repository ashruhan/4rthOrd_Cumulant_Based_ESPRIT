...
for Averaged_sample = 1:Averaged_samples;
    ...
    for n = 1:samples
        ...
        [~,kk]=sort(s1,'descend');
        ground_phase_est(Averaged_sample) = ground_phase_est(Averaged_sample) + 0.5*angle(c(kk(1),kk(1)))/samples;
        ground_mag_est(Averaged_sample) = ground_mag_est(Averaged_sample) + abs(c(kk(1),kk(1)))/samples;
        
        s2 = abs(Pol_Cum_vegitation'*u);
        [~,kk]=sort(s2,'descend');
        vegitation_phase_est(Averaged_sample) = vegitation_phase_est(Averaged_sample) + 0.5*angle(c(kk(1),kk(1)))/samples;
