...
for Averaged_sample = 1:Averaged_samples;
    ...
    for n = 1:samples
        ...
        signal_one_phase_est_second(Averaged_sample) = signal_one_phase_est_second(Averaged_sample) + (angle(c_2(kk_2(1),kk_2(1)))+signal_one_offset)/samples;
        ...
        signal_one_phase_est_fourth(Averaged_sample) = signal_one_phase_est_fourth(Averaged_sample) + (0.5*angle(c_4(kk_4(1),kk_4(1)))+signal_one_offset)/samples;
...
figure(2);title('Mean Error (dB)'); 
plot(SNR,signal_one_phase_est_second*180/pi,'b');
hold on;
plot(SNR,signal_one_phase_est_fourth*180/pi,'g');
hold off;
xlabel('SNR');ylabel('Mean Error (dB)')
