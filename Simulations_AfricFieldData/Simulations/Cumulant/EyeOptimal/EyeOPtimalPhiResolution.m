...
Glogal_DistLength = 250;
V_O_Dist = linspace(10,89,Glogal_DistLength);
vegetation_offset = V_O_Dist.*pi/180;    % veg interferomitry offset

eye_2 = linspace(-1,1,Glogal_DistLength);
eye_4 = linspace(-1,1,Glogal_DistLength);

ground_angle_4 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);
ground_angle_2 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);

vegitation_angle_4 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);
vegitation_angle_2 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);

ground_abs_4 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);
ground_abs_2 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);

vegitation_abs_4 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);
vegitation_abs_2 = zeros(SNR_Distribution,Glogal_DistLength,Glogal_DistLength);

%% Matrix Construction
for SNR_sample = 1:SNR_Distribution
    Noise = (10^(-SNR(SNR_sample)/20))/sqrt(3);
    for V_O_index = 1:Glogal_DistLength
        for eye_index = 1:Glogal_DistLength
            for unusedvariable = 1:Averaged_samples
                ...
                ground_angle_2(SNR_sample,V_O_index,eye_index) = ground_angle_2(V_O_index)...
                    + ((ground_offset - abs(angle(eigenval_2(srt_2(1),srt_2(1)))))^2)/Averaged_samples;
                
                ground_abs_2(SNR_sample,V_O_index,eye_index) = ground_abs_2(V_O_index)...
                    + abs(eigenval_2(srt_2(1),srt_2(1)))/Averaged_samples;
                
                polfilter_2 = abs(Pol_vegitation'*eigenvec_2);
                [~,srt_2] = sort(polfilter_2,'descend');
                vegitation_angle_2(SNR_sample,V_O_index,eye_index) = vegitation_angle_2(V_O_index)...
                    + ((vegetation_offset(V_O_index) - abs(angle(eigenval_2(srt_2(1),srt_2(1)))))^2)/Averaged_samples;
                
                vegitation_abs_2(SNR_sample,V_O_index,eye_index) = vegitation_abs_2(V_O_index)...
                    + abs(eigenval_2(srt_2(1),srt_2(1)))/Averaged_samples;
                ...
                ground_angle_4(SNR_sample,V_O_index,eye_index) = ground_angle_4(V_O_index)...
                    + ((ground_offset - abs(0.5*angle(eigenval_4(srt_4(1),srt_4(1)))))^2)/Averaged_samples;
                
                ground_abs_4(SNR_sample,V_O_index,eye_index) = ground_abs_4(V_O_index)...
                    + abs(eigenval_4(srt_4(1),srt_4(1)))/Averaged_samples;
                
                polfilter_4 = abs(Pol_Cum_vegitation'*eigenvec_4);
                [~,srt_4] = sort(polfilter_4,'descend');
                vegitation_angle_4(SNR_sample,V_O_index,eye_index) = vegitation_angle_4(V_O_index)...
                    + ((vegetation_offset(V_O_index) - abs(0.5*angle(eigenval_4(srt_4(1),srt_4(1)))))^2)/Averaged_samples;
                
                vegitation_abs_4(SNR_sample,V_O_index,eye_index) = vegitation_abs_4(V_O_index)...
                    + abs(eigenval_4(srt_4(1),srt_4(1)))/Averaged_samples;
            end
        end
    end
...
%% Plotting Results
figure(1);
meshc(ground_angle_4(30,:,:));title('4th order ground RMS Error');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(2);
meshc(vegitation_angle_4(30,:,:));title('4th order vegetation RMS Error');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(3)
meshc(ground_angle_2(30,:,:));title('2nd order ground RMS Error');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(4)
meshc(vegitation_angle_2(30,:,:));title('2nd order vegetation RMS Error');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(5)
meshc(ground_abs_4(30,:,:));title('4th order ground coherance');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(6)
meshc(vegitation_abs_4(30,:,:));title('4th order vegetation coherance');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(7)
meshc(ground_abs_2(30,:,:));title('2nd order ground coherance');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');
figure(8)
meshc(vegitation_abs_2(30,:,:));title('2nd order vegetation coherance');
xlabel('vegetation sweep 1-89 degrees');ylabel('ground sweep 1-89 degrees');